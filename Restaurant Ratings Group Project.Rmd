...
Restaurants_final <- subset(Restaurants_v5,!duplicated(business_id),-c(17))
...
Stars_ReviewCount_LogitModel <- polr(as.factor(stars) ~ review_count, data=Restaurants_final, Hess = TRUE)
Stars_Checkin_LogitModel <- polr(as.factor(stars) ~ checkin_count, data=Restaurants_final, Hess = TRUE)
Stars_Location_LogitModel <- polr(as.factor(stars) ~ postal_code, data=Restaurants_final, Hess = TRUE)


summary(Stars_Location_LogitModel)
...
qplot(data=Restaurants_final,review_count ,bin=50)
